...
<div class="top-nav"><div class="nav-inner">
    <div class="nav-left">
        <a href="?menu=home&auth=1" class="nav-logo-link" onclick="event.preventDefault(); window.location.search='?menu=home&auth=1';"><div class="logo-t">IGLOO</div><div class="logo-s">AI MODEL HUB</div></a>
        <div class="nav-menu">
            <a href="?menu=notice&auth=1" class="{_ac('notice')}" onclick="event.preventDefault(); window.location.search='?menu=notice&auth=1';">공지사항</a>
            <a href="?menu=models&page=list&auth=1" class="{_ac('models')}" onclick="event.preventDefault(); window.location.search='?menu=models&page=list&auth=1';">Models</a>
            <a href="?menu=docs&auth=1" class="{_ac('docs')}" onclick="event.preventDefault(); window.location.search='?menu=docs&auth=1';">Docs</a>
        </div>
    </div>
    <div class="nav-right">
        <div class="pc">
            ...
            <div class="dd-wrap"><div class="dd-menu">
                ...
                <a href="?menu=management&auth=1" onclick="event.preventDefault(); window.location.search='?menu=management&auth=1';">➕ Model Management</a>
                <a href="?menu=docs_write&auth=1" onclick="event.preventDefault(); window.location.search='?menu=docs_write&auth=1';">➕ Docs</a>
                <div class="dd-div"></div>
                <a href="?logout=1" onclick="event.preventDefault(); window.location.search='?logout=1';">🚪 로그아웃</a>
            </div></div>
        </div>
    </div>
</div></div>
...
def _card(m, created=False):
    ...
    return f"""<a href="?menu=models&page=detail&model_id={m['id']}&auth=1" class="mc" onclick="event.preventDefault(); window.location.search='?menu=models&page=detail&model_id={m['id']}&auth=1';">
...
if menu == "home":
    ...
    cl, cr = st.columns(2)
    with cl:
        st.markdown('<div class="sec-h"><div class="sec-t">Recently Added <span class="sec-ts">최근 등록</span></div><a href="?menu=models&page=list&sort=created&auth=1" class="va-link" onclick="event.preventDefault(); window.location.search=\'?menu=models&page=list&sort=created&auth=1\';">전체보기 →</a></div>', unsafe_allow_html=True)
    ...
    with cr:
        st.markdown('<div class="sec-h"><div class="sec-t">Recently Updated <span class="sec-ts">최근 업데이트</span></div><a href="?menu=models&page=list&sort=updated&auth=1" class="va-link" onclick="event.preventDefault(); window.location.search=\'?menu=models&page=list&sort=updated&auth=1\';">전체보기 →</a></div>', unsafe_allow_html=True)
    ...
    with ct:
        ...
        if tp > 1:
            ...
            for pn in range(max(1,cp-2), min(tp,cp+2)+1):
                ...
                ph += f'<span class="pg-b on">{pn}</span>' if pn==cp else f'<a href="{u}" class="pg-b" onclick="event.preventDefault(); window.location.search=\'{u}\';">{pn}</a>'
            ph += '</div>'
